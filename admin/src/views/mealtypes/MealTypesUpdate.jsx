...
const MealTypesUpdate = () => {
  ...
  React.useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(`meals/types/${id}`, {
    ...
    fetchData();
  }, [id, setValue, setMealTypes]);

  const handleFormSubmit = async (data, property) => {
    try {
      const response = await axios.put(`meals/types/${id}`, {
        property,
      ...
      <Box sx={{ width: '100%' }}>
        {!loading ? (
          <React.Fragment>
            ...
            >
              <StyledBoxContainer text='Meal Type Name'>
                {nameVisible ? (
                  <Box
                    sx={{
                      display: 'flex',
                  ...
                  >
                    ...
                    <Tooltip title='Cancel'>
                      <IconButton
                        size='small'
                        onClick={() => {
                          setValue('description', mealTypes?.description);
...
export default MealTypesUpdate;
